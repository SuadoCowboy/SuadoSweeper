...
class MultiplayerInterface(GenericUI):
    def __init__(self):
        super().__init__(WIDTH//2-WIDTH//4, HEIGHT//2-HEIGHT//4,WIDTH//2,HEIGHT//2)
        
        self.address_input = TextInputLine(5, self.rect.height//2, self.rect.width-10, FONT_SIZE)
        
        self.connect_button = Button(self.address_input.rect.centerx-50, self.rect.height//2+FONT_SIZE+2, 100, 30)
        self.connect_button.background_color = (255,255,255)

    def update(self, mouse_pos: tuple[int, int], keyDownEvent, mouseButtonDownEvent):
        self.address_input.update()
        
        if keyDownEvent:
            self.address_input.keydown_event(keyDownEvent)
        
        if mouseButtonDownEvent:
            self.address_input.rect.x += self.rect.x
            self.address_input.rect.y += self.rect.y

            self.address_input.mousebuttondown_event(mouseButtonDownEvent)

            self.address_input.rect.x -= self.rect.x
            self.address_input.rect.y -= self.rect.y
        
        self.connect_button.rect.x += self.rect.x
        self.connect_button.rect.y += self.rect.y
        ...
        if self.connect_button.pressed:
            print('Connect code here(search for #MULTIPLAYER in UI.py)') #MULTIPLAYER
    
    def draw(self, surface: pygame.Surface):
        surface.blit(self, self.rect)
        self.fill(self.background_color)

        self.address_input.draw(self)
        self.connect_button.draw(self)